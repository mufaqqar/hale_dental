<section class="w-full bg-[#f5f5f5] py-12">

    <div class="container mx-auto px-4 py-5 bg-white rounded-[23px] grid md:grid-cols-8 grid-cols-2 items-center justify-between gap-6 shadow-[0_10px_30px_rgba(0,0,0,0.05)]">
        <div class="flex items-center justify-center h-[35px] w-[117px]">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/images/1.svg" alt="" width="117" height="35"
                class="w-full h-full" />
        </div>
        <div class="flex items-center justify-center h-[35px] w-[117px]">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/images/2.svg" alt="" width="117" height="35"
                class="w-full h-full" />
        </div>
        <div class="flex items-center justify-center h-[35px] w-[117px]">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/images/3.webp" alt="" width="117" height="35"
                class="w-full h-full" />
        </div>
        <div class="flex items-center justify-center h-[35px] w-[117px]">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/images/4.svg" alt="" width="117" height="35"
                class="w-full h-full" />
        </div>
        <div class="flex items-center justify-center h-[35px] w-[117px]">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/images/5.webp" alt="" width="117" height="35"
                class="w-full h-full" />
        </div>
        <div class="flex items-center justify-center h-[35px] w-[117px]">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/images/6.svg" alt="" width="117" height="35"
                class="w-full h-full" />
        </div>
        <div class="flex items-center justify-center h-[35px] w-[117px]">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/images/7.svg" alt="" width="117" height="35"
                class="w-full h-full" />
        </div>
        <div class="flex items-center justify-center h-[35px] w-[117px]">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/images/8.svg" alt="" width="117" height="35"
                class="w-full h-full" />
        </div>

    </div>

    <div class="container mx-auto px-4 text-center mt-6">
        <p class="m-0 text-[16px] leading-[1.55] text-[#171717] font-normal">
            We utilise NIQ standards for data-driven insights and patient satisfaction analysis.
            This partnership validates our transparency and our commitment to
            continuously improving the patient experience through internationally verified data.
        </p>
    </div>

    <div class="container mx-auto px-4 text-center mt-[82px] mb-[22px]">
        <h2 class="m-0 text-black text-[46px] leading-none font-bold tracking-[-2px]">
            Start Your Transformation
        </h2>
    </div>
    <div class="container mx-auto px-4 grid grid-cols-6 gap-5">
        <div
            class="h-[177px] bg-white rounded-[12px] overflow-hidden shadow-[0_3px_9px_rgba(0,0,0,0.08)] flex flex-col">
            <div class="flex-1 p-3">
                <div class="w-[34px] h-[34px] rounded-[8px] bg-[#f4f4f4] flex items-center justify-center mb-[11px]">
                    <svg class="w-[18px] h-[18px] text-[#4c7df5]" viewBox="0 0 24 24" fill="currentColor">
                        <path
                            d="M12 3.5c-2.1-2-5.7-1.7-7.5.7-1.5 2-1.3 4.4-.6 6.4.7 2 1.4 3.6 1.7 6.3.3 2.5 1.1 4.2 2.5 4.2 1.3 0 1.5-1.4 1.7-3.2.2-1.5.5-2.5 1.2-2.5s1 .9 1.2 2.5c.2 1.8.4 3.2 1.7 3.2 1.4 0 2.2-1.7 2.5-4.2.3-2.7 1-4.3 1.7-6.3.7-2 .9-4.4-.6-6.4C17.7 1.8 14.1 1.5 12 3.5z" />
                    </svg>
                </div>
                <h3 class="m-0 text-[18px] leading-[1.15] font-semibold text-[#303030]">
                    Hollywood Smile
                </h3>
                <p class="m-0 mt-[5px] text-[15px] leading-[1.05] italic text-[#a6a6a6]">
                    A natural, bright smile<br>
                    with premium materials.
                </p>
            </div>
            <a href="#"
                class="h-[33px] shrink-0 flex items-center justify-center bg-[#4c7df5] hover:bg-[#3769e8] text-white text-[12px] font-semibold no-underline">
                Select
            </a>
        </div>
        <div
            class="h-[177px] bg-white rounded-[12px] overflow-hidden shadow-[0_3px_9px_rgba(0,0,0,0.08)] flex flex-col">
            <div class="flex-1 p-3">

                <div class="w-[34px] h-[34px] rounded-[8px] bg-[#f4f4f4] flex items-center justify-center mb-[11px]">
                    <svg class="w-[18px] h-[18px] text-[#4c7df5]" viewBox="0 0 24 24" fill="currentColor">
                        <path
                            d="M12 3.5c-2.1-2-5.7-1.7-7.5.7-1.5 2-1.3 4.4-.6 6.4.7 2 1.4 3.6 1.7 6.3.3 2.5 1.1 4.2 2.5 4.2 1.3 0 1.5-1.4 1.7-3.2.2-1.5.5-2.5 1.2-2.5s1 .9 1.2 2.5c.2 1.8.4 3.2 1.7 3.2 1.4 0 2.2-1.7 2.5-4.2.3-2.7 1-4.3 1.7-6.3.7-2 .9-4.4-.6-6.4C17.7 1.8 14.1 1.5 12 3.5z" />
                    </svg>
                </div>

                <h3 class="m-0 text-[18px] leading-[1.15] font-semibold text-[#303030]">
                    Dental Implants
                </h3>

                <p class="m-0 mt-[5px] text-[15px] leading-[1.05] italic text-[#a6a6a6]">
                    Permanent<br>
                    replacements for<br>
                    missing teeth.
                </p>

            </div>

            <a href="#"
                class="h-[33px] shrink-0 flex items-center justify-center bg-[#4c7df5] hover:bg-[#3769e8] text-white text-[12px] font-semibold no-underline">
                Select
            </a>

        </div>


        <!-- CARD -->
        <div
            class="h-[177px] bg-white rounded-[12px] overflow-hidden shadow-[0_3px_9px_rgba(0,0,0,0.08)] flex flex-col">

            <div class="flex-1 p-3">

                <div class="w-[34px] h-[34px] rounded-[8px] bg-[#f4f4f4] flex items-center justify-center mb-[11px]">
                    <svg class="w-[18px] h-[18px] text-[#4c7df5]" viewBox="0 0 24 24" fill="currentColor">
                        <path
                            d="M12 3.5c-2.1-2-5.7-1.7-7.5.7-1.5 2-1.3 4.4-.6 6.4.7 2 1.4 3.6 1.7 6.3.3 2.5 1.1 4.2 2.5 4.2 1.3 0 1.5-1.4 1.7-3.2.2-1.5.5-2.5 1.2-2.5s1 .9 1.2 2.5c.2 1.8.4 3.2 1.7 3.2 1.4 0 2.2-1.7 2.5-4.2.3-2.7 1-4.3 1.7-6.3.7-2 .9-4.4-.6-6.4C17.7 1.8 14.1 1.5 12 3.5z" />
                    </svg>
                </div>

                <h3 class="m-0 text-[18px] leading-[1.15] font-semibold text-[#303030]">
                    Dental Crown
                </h3>

                <p class="m-0 mt-[5px] text-[15px] leading-[1.05] italic text-[#a6a6a6]">
                    Custom-fit restorations<br>
                    for strength and<br>
                    comfort.
                </p>

            </div>

            <a href="#"
                class="h-[33px] shrink-0 flex items-center justify-center bg-[#4c7df5] hover:bg-[#3769e8] text-white text-[12px] font-semibold no-underline">
                Select
            </a>

        </div>


        <!-- CARD -->
        <div
            class="h-[177px] bg-white rounded-[12px] overflow-hidden shadow-[0_3px_9px_rgba(0,0,0,0.08)] flex flex-col">

            <div class="flex-1 p-3">

                <div class="w-[34px] h-[34px] rounded-[8px] bg-[#f4f4f4] flex items-center justify-center mb-[11px]">
                    <svg class="w-[18px] h-[18px] text-[#4c7df5]" viewBox="0 0 24 24" fill="currentColor">
                        <path
                            d="M12 3.5c-2.1-2-5.7-1.7-7.5.7-1.5 2-1.3 4.4-.6 6.4.7 2 1.4 3.6 1.7 6.3.3 2.5 1.1 4.2 2.5 4.2 1.3 0 1.5-1.4 1.7-3.2.2-1.5.5-2.5 1.2-2.5s1 .9 1.2 2.5c.2 1.8.4 3.2 1.7 3.2 1.4 0 2.2-1.7 2.5-4.2.3-2.7 1-4.3 1.7-6.3.7-2 .9-4.4-.6-6.4C17.7 1.8 14.1 1.5 12 3.5z" />
                    </svg>
                </div>

                <h3 class="m-0 text-[18px] leading-[1.15] font-semibold text-[#303030]">
                    Veneers
                </h3>

                <p class="m-0 mt-[5px] text-[15px] leading-[1.05] italic text-[#a6a6a6]">
                    Aligns teeth and jaws<br>
                    orthodontics.
                </p>

            </div>

            <a href="#"
                class="h-[33px] shrink-0 flex items-center justify-center bg-[#4c7df5] hover:bg-[#3769e8] text-white text-[12px] font-semibold no-underline">
                Select
            </a>

        </div>


        <!-- CARD -->
        <div
            class="h-[177px] bg-white rounded-[12px] overflow-hidden shadow-[0_3px_9px_rgba(0,0,0,0.08)] flex flex-col">

            <div class="flex-1 p-3">

                <div class="w-[34px] h-[34px] rounded-[8px] bg-[#f4f4f4] flex items-center justify-center mb-[11px]">
                    <svg class="w-[18px] h-[18px] text-[#4c7df5]" viewBox="0 0 24 24" fill="currentColor">
                        <path
                            d="M12 3.5c-2.1-2-5.7-1.7-7.5.7-1.5 2-1.3 4.4-.6 6.4.7 2 1.4 3.6 1.7 6.3.3 2.5 1.1 4.2 2.5 4.2 1.3 0 1.5-1.4 1.7-3.2.2-1.5.5-2.5 1.2-2.5s1 .9 1.2 2.5c.2 1.8.4 3.2 1.7 3.2 1.4 0 2.2-1.7 2.5-4.2.3-2.7 1-4.3 1.7-6.3.7-2 .9-4.4-.6-6.4C17.7 1.8 14.1 1.5 12 3.5z" />
                    </svg>
                </div>

                <h3 class="m-0 text-[18px] leading-[1.15] font-semibold text-[#303030]">
                    Teeth Whitening
                </h3>

                <p class="m-0 mt-[5px] text-[15px] leading-[1.05] italic text-[#a6a6a6]">
                    Removes deep stains for<br>
                    a lasting shine.
                </p>

            </div>

            <a href="#"
                class="h-[33px] shrink-0 flex items-center justify-center bg-[#4c7df5] hover:bg-[#3769e8] text-white text-[12px] font-semibold no-underline">
                Select
            </a>

        </div>


        <!-- FIND ALL -->
        <a href="#"
            class="h-[177px] rounded-[12px] bg-[#4c7df5] hover:bg-[#3769e8] p-3 flex flex-col justify-between text-white no-underline transition-colors">

            <div class="w-[34px] h-[34px] rounded-[8px] bg-white flex items-center justify-center">

                <svg class="w-[18px] h-[18px] text-[#4c7df5]" viewBox="0 0 24 24" fill="currentColor">
                    <path
                        d="M12 3.5c-2.1-2-5.7-1.7-7.5.7-1.5 2-1.3 4.4-.6 6.4.7 2 1.4 3.6 1.7 6.3.3 2.5 1.1 4.2 2.5 4.2 1.3 0 1.5-1.4 1.7-3.2.2-1.5.5-2.5 1.2-2.5s1 .9 1.2 2.5c.2 1.8.4 3.2 1.7 3.2 1.4 0 2.2-1.7 2.5-4.2.3-2.7 1-4.3 1.7-6.3.7-2 .9-4.4-.6-6.4C17.7 1.8 14.1 1.5 12 3.5z" />
                </svg>

            </div>

            <span class="text-[18px] leading-[1.2] font-medium">
                Find All<br>
                Dental Treatments
            </span>

        </a>

    </div>
    <div class="text-center mt-[18px] text-[20px] leading-[1.3] text-[#171717]">
        Tell us what treatments you're looking for. We'll build you a bespoke package.
    </div>
</section>